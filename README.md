...
* `Invoke-PSADSession`: Reflectively loads the Active Directory PowerShell module assembly into a remote PowerShell session and enters it, bypassing the need to add it as an optional Windows feature on the target system. Because this module is Microsoft-signed, it's much easier to avoid detection with.
* More to come